...
function LootMaster:ShowUpdateFrame( sender, iVersion, sVersion )
    ...
    message( string.format(L["Auto notice from %s: please update epgp_lootmaster from curse.com. If you fail to do so you might not get loot during raids."], sender, sVersion ) )
end
...
function LootMaster:EnterCombat()
    ...
    if self.frame and self.frame:IsShown() then
        self:Print(L["You've entered combat, hiding interface."]);
...
function LootMaster:SaveNote( frame, keepVisibility )
    ...
    if frame.data.note and frame.data.note~='' then
        frame.lblNote:SetText(format(L['My note: %s'], frame.data.note or ''))
    else
...
function LootMaster:ShowInfoPopup( ... )
...
function LootMaster:HideInfoPopup()
	GameTooltip:Hide()
end
...
function LootMaster:CreateLootSelectFrame()
	...
	local lblBid = frame:CreateFontString(nil,"OVERLAY","GameFontNormalSmall")
	lblBid:SetPoint("BOTTOMLEFT", icon, "BOTTOMRIGHT", 5, 7)
    lblBid:SetVertexColor( 0.9, 0.9, 0.9 );
    lblBid:SetText( L["Your GP Bid:"] )
    frame.lblBid = lblBid;

	local tbGPValueFrame = CreateFrame("Frame", nil, frame)
	tbGPValueFrame:SetHeight(20)
    tbGPValueFrame:SetWidth(65);
	...
	tbGPValueFrame:SetPoint("BOTTOMLEFT", lblBid, "BOTTOMRIGHT", 5, -5)
    ...
    tbGPValue:SetFontObject('GameFontHighlight')
    ...
    tbGPValue:SetScript("OnEditFocusLost", function()
		...
		local gpValue = tonumber(frame.data.gpvalue) or 0
        iGPBid = tonumber(tbGPValue:GetText()) or frame.data.gpbid or gpValue
		if iGPBid<gpValue then iGPBid=gpValue end
        tbGPValue:SetText(iGPBid);
        frame.data.gpbid = iGPBid;
    end)
    ...
    btnPass:SetScript("OnClick", function()
              self:SaveNote(frame)
              self:SendItemWanted( frame.data.lootmaster, frame.data.id, self.RESPONSE.PASS, frame.data.note, 0 )
              self:RemoveLoot( frame.data.id );
              self:UpdateLootUI();
    end)
    btnPass:SetPoint("BOTTOMLEFT",btnGreed,"BOTTOMRIGHT",5,0)
    btnPass:SetText(L["Pass"])
    ...
    b:SetScript("OnUpdate", function( o, elapsed )
        if not frame or not frame.data or not frame.data.timeoutLeft then
            ...
            frame.lblTimeout:SetText( L['No timeout'] )
            return;
        end
        ...
        if frame.data.timeoutLeft<0 then
            ...
            b:SetValue(0);
            frame.lblTimeout:SetText( L['No time left'] )

            self:SaveNote(frame)
            self:SendItemWanted( frame.data.lootmaster, frame.data.id, self.RESPONSE.TIMEOUT, frame.data.note, 0 )
            self:RemoveLoot( frame.data.id );
            self:UpdateLootUI();
        ...
        frame.lblTimeout:SetText( format(L["%s secs until pass"], ceil(frame.data.timeoutLeft)) )
    ...
    btnNote:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine(L["Click this to add a note to send to the master looter."],.8,.8,.8,1, true)
        GameTooltip:Show()
    end)
...
function LootMaster:UpdateLootUI()
    ...
    for i, data in ipairs(self.lootList) do repeat
		...
		lootFrame.tbGPValue:SetText(data.gpbid or data.gpvalue or 0)
        ...
        if data.gpvalue2 and data.gpvalue2~='' then
            gp = gp .. format(L[' or %s'], data.gpvalue2)
        end
		...
		if data.bidsAllowed then
			lootFrame.lblBid:Show()
			lootFrame.tbGPValueFrame:Show()
			lastButton = lootFrame.tbGPValueFrame
			totalButtonWidth = lootFrame.lblBid:GetWidth() + lootFrame.tbGPValueFrame:GetWidth() + 20
		else
			lootFrame.lblBid:Hide()
			lootFrame.tbGPValueFrame:Hide()
		end
        ...
        for i=1, data.numButtons do
          ...
          if not button then
            ...
            button:SetScript("OnClick", function(btn)
				...
				if lootFrame.data.bidsAllowed then
					self:SendItemWanted(lootFrame.data.lootmaster, lootFrame.data.id, btn.response, lootFrame.data.note, tonumber(lootFrame.tbGPValue:GetText()) or 0)
				else
					self:SendItemWanted(lootFrame.data.lootmaster, lootFrame.data.id, btn.response, lootFrame.data.note, 0)
				end
		  ...
		  if lastButton==lootFrame.tbGPValueFrame then
			button:SetPoint("BOTTOMLEFT", lastButton, "BOTTOMRIGHT", 5, -2)
		  else
        ...
        lootFrame.lblInfo:SetText(format(L["ilevel: %s, GP: %s"], data.ilevel or -1, gp))
        lootFrame.lblLooter:SetText(format(L["looter: %s"], data.lootmaster or L['unknown']))
...
function LootMaster:ShowVersionCheckFrame()
    ...
    if not frame then
        ...
        lblVersionCheck:SetText( L["Request version for: "] )
