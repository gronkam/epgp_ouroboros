...
function LootMaster:EnterCombat()
    ...
    if self.frame and self.frame:IsShown() then
        self:Print("You've entered combat, hiding interface.");
...
function LootMaster:SaveNote( frame, keepVisibility )
    ...
    if frame.data.note and frame.data.note~='' then
        frame.lblNote:SetText(format('My note: %s', frame.data.note or ''))
    else
...
function LootMaster:CreateLootSelectFrame()
    ...
    b:SetScript("OnUpdate", function( o, elapsed )
        ...
        if frame.data.timeoutLeft<0 then
            ...
            b:SetValue(0);
            frame.lblTimeout:SetText( 'No time left' )
        ...
        frame.lblTimeout:SetText( format("%s секунд до паса", ceil(frame.data.timeoutLeft)) )
    ...
    btnNote:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Добавить пометку")
        GameTooltip:AddLine("Щелкните для добавления примечания, чтобы послать Лидеру.",.8,.8,.8,1, true)
        GameTooltip:Show()
    end)
...
function LootMaster:UpdateLootUI()
    ...
    for i, data in ipairs(self.lootList) do repeat
        ...
        lootFrame.lblInfo:SetText(format("Уровень: %s, GP: %s", data.ilevel or -1, gp))
        lootFrame.lblLooter:SetText(format("Лутает: %s", data.lootmaster or 'unknown'))
